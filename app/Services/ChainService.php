...
class ChainService
{
    private function ensureMockChainWritesEnabled(): void
    {
        $enabled = (bool) config('blockchain.mock_transactions_enabled', true);

        if (! $enabled) {
            throw new \RuntimeException('Mock chain writes are disabled. Configure real chain integration before enabling this flow.');
        }
    }
    ...
    public function mintEditionToUserWallet(ProductEdition $edition, Wallet $wallet): ChainToken
    {
        ...
        $this->ensureMockChainWritesEnabled();
