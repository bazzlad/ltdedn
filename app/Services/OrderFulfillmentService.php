...
class OrderFulfillmentService
{
    ...
    public function markShipped(Order $order, string $carrier, string $tracking, User $actor): array
    {
        ...
        DB::transaction(function () use ($order, $carrier, $tracking, $actor, &$error) {
            ...
            if ($locked->status !== OrderStatus::Paid) {
                $error = 'Order must be paid before it can be shipped.';

                return;
            }

            if ((int) $locked->refunded_amount >= (int) $locked->total_amount && (int) $locked->total_amount > 0) {
                $error = 'Order has been fully refunded and cannot be shipped.';
            ...
            if ($isFirstShip) {
                DB::afterCommit(function () use ($locked) {
                    $fresh = $locked->fresh();
                    $this->dispatchShippedNotification($fresh);
                    $this->dispatchPushback($fresh);
                });
            }
        });
    ...
    private function dispatchPushback(?Order $order): void
    {
