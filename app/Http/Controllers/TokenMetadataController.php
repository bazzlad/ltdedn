...
class TokenMetadataController extends Controller
{
    ...
    {
        $chainToken = ChainToken::where('token_id', $tokenId)
            ->where('chain', (string) config('blockchain.chain', 'polygon'))
            ->where('contract_address', (string) config('blockchain.contract_address'))
            ->firstOrFail();
