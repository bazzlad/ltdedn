            ...
            <div class="w-full max-w-md">
                ...
                <div class="mt-4 grid grid-cols-2 gap-4 rounded-sm bg-neutral-900/80 p-4 ring-1 ring-white/10">
                    <div>
                        ...
                        <p class="text-2xl font-extrabold tracking-tight text-white sm:text-3xl">{{ editionNumber }} / {{ editionSize }}</p>
                    </div>
                    <div>
                ...
                <!-- Ownership Status -->
                <div v-if="isClaimed" class="mt-4 rounded-sm border border-blue-500/30 bg-blue-500/10 p-3 ring-1 ring-blue-500/20">
                ...
                <div class="mt-4 space-y-4">
                    ...
                    <div v-if="canClaim">
                        ...
                        <div v-else class="text-center">
                            <div class="mb-4 rounded-sm border border-yellow-500/30 bg-yellow-500/10 p-3 ring-1 ring-yellow-500/20">
                                <p class="text-sm font-semibold text-yellow-200">You need to be logged in to claim this edition</p>
                            </div>
                            <div class="flex flex-col gap-2.5 sm:flex-row sm:gap-3">
                                ...
                                <Link
                                    ...
                                    class="inline-flex flex-1 items-center justify-center border border-white/30 px-4 py-3 text-sm font-bold tracking-wider text-white transition-all hover:border-white/50 hover:bg-white/10 active:scale-[0.98] sm:px-6 sm:text-base"
                                >
                                    SIGN UP
                                </Link>
                            </div>
                    ...
                    <div
                        v-if="isOwnedByCurrentUser && activeTransfer"
                        class="rounded-sm border border-yellow-500/30 bg-yellow-500/10 p-4 ring-1 ring-yellow-500/20"
                    >
                        <div class="mb-3 text-center">
                            ...
                            <p class="mt-0.5 text-xs text-yellow-200/80">Waiting for {{ activeTransfer.recipient.name }} to accept.</p>
                        </div>
                        <Form :action="transfersCancel(activeTransfer.token).url" method="post" #default="{ processing, submit }">
                            <Button
                                type="button"
                                variant="destructive"
                    ...
                    <div v-else-if="isOwnedByCurrentUser">
                        <details class="overflow-hidden rounded-sm border border-white/10 bg-neutral-900/50 ring-1 ring-white/5">
                            ...
                            <div class="border-t border-white/10 p-3">
                                <Form :action="qrTransfer(edition.qr_code).url" method="post" class="space-y-3" #default="{ processing }">
                                    <div>
                                        ...
                                        <Input
                                            ...
                                            class="mt-1.5"
                                        />
                                        <p class="mt-1.5 text-xs text-neutral-500">Recipient must have an existing account.</p>
                                    </div>
                                    <Button
                                        type="submit"
                                        variant="outline"
